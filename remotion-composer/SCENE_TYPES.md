...
| **`timeline`** | **`TimelineScene`** | **`milestones`** (list of `{label, sub?, color?}`) | `title`, `accentColor`, `backgroundColor` | **Animated horizontal timeline: line draws, milestone dots + labels pop in sequence across the scene duration — for history/chain stories** |
